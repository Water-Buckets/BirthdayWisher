...
if __name__ == "__main__":
    ...
    try:
        ...
        if today_birthdays or weekend_birthdays:
            ...
            wish = ('' if not today_birthdays else f"祝{'、'.join([entry['name'] for entry in today_birthdays])} {datetime.date.today().year - datetime.datetime.strptime(today_birthdays[0]['birthday'], '%Y-%m-%d').year} 岁生日快乐!") + ('' if not weekend_birthdays else ('' if not today_birthdays else '\n\n') + "\n\n".join([f"提前祝{entry['name']} {datetime.date.today().year - datetime.datetime.strptime(entry['birthday'], '%Y-%m-%d').year} 岁生日快乐! ("+ str(datetime.datetime.strptime(entry['birthday'], '%Y-%m-%d').month) + "." +str(datetime.datetime.strptime(entry['birthday'], '%Y-%m-%d').day)+")" for entry in weekend_birthdays]))
            draw = PIL.ImageDraw.Draw(image)
            draw.text((width // 2, height // 2), wish, font=PIL.ImageFont.truetype(config['font_path'], config['font_size']), fill=config['font_color'], anchor='mm')
            draw.text((width - 40, 40), f"以防你不知道\n\n距离 2026.6.7 还有\n\n {days_remaining} 天", font=PIL.ImageFont.truetype(config['font_path'], config['font_size']//2), fill=config['font_color'], anchor='ra',align='right')
            ...
            draw = PIL.ImageDraw.Draw(image)
            draw.text((width - 40, 40), f"以防你不知道\n\n距离 2026.6.7 还有\n\n {days_remaining} 天",
                      font=PIL.ImageFont.truetype(config['font_path'], config['font_size'] // 2),
                      fill=config['font_color'], anchor='ra',align='right')
            image.save(os.path.join(os.path.dirname(config['template_image_path']), 'birthday_background.png'))
            ctypes.windll.user32.SystemParametersInfoW(20, 0, os.path.abspath(os.path.join(os.path.dirname(config['template_image_path']), 'birthday_background.png')), 3)
